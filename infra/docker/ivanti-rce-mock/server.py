"""Mock Ivanti Connect Secure — CVE-2024-21887 command injection behaviour.

Extends CVE-2023-46805 auth bypass: when vuln and URL traverses to
/license/keys-status/ with a semicolon-injected command, executes it and
echoes output in the response.
"""
import json
import os
import subprocess
...
SYSTEM_INFO = json.dumps({
    ...
    "software-inventory": ["ivanti-ics-22.5.0.0"],
}).encode()

IVANTI_ROOT = b'<html><head><title>Ivanti Connect Secure</title></head><body><a href="welcome.cgi?p=logo">Login</a></body></html>'


def _extract_injected_cmd(raw_path):
    """URL-decode path and extract shell command after the first semicolon."""
...
def _run_cmd(cmd):
    """Execute command and return stdout (capped at 512 bytes)."""
    try:
...
def _handler_class():
    ...
    class IvantiRCEHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if ".." in raw:
                ...
                if "license/keys-status/" in raw:
                    cmd = _extract_injected_cmd(raw)
                    if cmd:
                        output = _run_cmd(cmd)
                    else:
                        output = ""
...
if __name__ == "__main__":
    ...
    print(f"Ivanti RCE mock on :80/:443 (mode={mode})", flush=True)
